...
def validate(model, data_loader, device, with_pose):
    ...
    cos_baseline_meter_embmean = AverageMeter()
    cos_baseline_meter_cosmean = AverageMeter()
    cos_baseline_meter_cosmax = AverageMeter()
    ...
    with torch.no_grad():
        for embeddings, labels, scan_ids, true_poses, _, path in data_loader:
            ...
            # available non-front indices
            non_front_mask = ~is_front

            # sample two different non-front views
            idx1 = []
            idx2 = []
            for b in range(B):
                candidates = torch.where(non_front_mask[b])[0]
                choice = candidates[torch.randperm(len(candidates))[:2]]
                idx1.append(choice[0])
                idx2.append(choice[1])

            idx1 = torch.stack(idx1)
            idx2 = torch.stack(idx2)

            input_emb1 = embeddings[torch.arange(B), idx1].to(device)
            input_emb2 = embeddings[torch.arange(B), idx2].to(device)

            # if with_pose:
            #    pred_front = model(input_emb, input_pose)
            # else:
            pred_front = model(input_emb1, input_emb2)
            ...
            # --- baseline: mean of two embedding input views vs front ---
            baseline_emb = 0.5 * (input_emb1 + input_emb2)
            cos_baseline_embmean = F.cosine_similarity(
                F.normalize(baseline_emb, dim=-1),
                F.normalize(front_emb, dim=-1),
                dim=-1
            ).mean()
            # --- baseline: mean and max of two cosine distance input views vs front ---
            cos1 = F.cosine_similarity(
                F.normalize(input_emb1, dim=-1),
                F.normalize(front_emb, dim=-1),
                dim=-1
            )
            cos2 = F.cosine_similarity(
                F.normalize(input_emb2, dim=-1),
                F.normalize(front_emb, dim=-1),
                dim=-1
            )
            ...
            cos_baseline_meter_cosmax.update(cos_baseline_cosmax.item(), B)
            cos_baseline_meter_cosmean.update(cos_baseline_cosmean.item(), B)

    return {
        "cosine_model": cos_model_meter.avg,
        "cosine_baseline_embmean": cos_baseline_meter_embmean.avg,
        "cosine_baseline_cosmean": cos_baseline_meter_cosmean.avg,
        "cosine_baseline_cosmax": cos_baseline_meter_cosmax.avg,
        "cosine_gain_embmean": cos_model_meter.avg - cos_baseline_meter_embmean.avg,
        "cosine_gain_cosmean": cos_model_meter.avg - cos_baseline_meter_cosmean.avg,
        "cosine_gain_cosmax": cos_model_meter.avg - cos_baseline_meter_cosmax.avg,
    }
...
def main(cfg):
    ...
    with mlflow.start_run(run_name=f"{RUN_NAME}_[{run_count + 1}]") as run:
        ...
        # ======= Predictor =======
        predictor_dict = {'PoseFrontalizer': lambda: PoseFrontalizerTwoView(),}
        ...
        #    )
        # else:
        dummy1 = torch.zeros(BATCH_SIZE, EMBEDDING_SIZE).to(DEVICE)
        dummy2 = torch.zeros(BATCH_SIZE, EMBEDDING_SIZE).to(DEVICE)
        model_stats_predictor = summary(
            predictor,
            input_data=(dummy1, dummy2),
            verbose=0
        )
        ...
        mlflow.log_metric('val_cosine_baseline_embmean', val_metrics["cosine_baseline_embmean"], step=0)
        mlflow.log_metric('val_cosine_gain_embmean', val_metrics["cosine_gain_embmean"], step=0)
        mlflow.log_metric('val_cosine_baseline_cosmean', val_metrics["cosine_baseline_cosmean"], step=0)
        mlflow.log_metric('val_cosine_gain_cosmean', val_metrics["cosine_gain_cosmean"], step=0)
        mlflow.log_metric('val_cosine_baseline_cosmax', val_metrics["cosine_baseline_cosmax"], step=0)
        mlflow.log_metric('val_cosine_gain_cosmax', val_metrics["cosine_gain_cosmax"], step=0)
        ...
        for epoch in range(0, NUM_EPOCH):
            ...
            for step, (embeddings, labels, scan_ids, true_poses, _, path) in enumerate(tqdm(train_loader)):
                ...
                #    pred_front = predictor(input_emb1, input_emb2)
                # else:
                pred_front = predictor(input_emb1, input_emb2)
            ...
            mlflow.log_metric('val_cosine_prediction', val_metrics["cosine_model"], step=0)
            mlflow.log_metric('val_cosine_baseline_embmean', val_metrics["cosine_baseline_embmean"], step=epoch + 1)
            mlflow.log_metric('val_cosine_gain_embmean', val_metrics["cosine_gain_embmean"], step=epoch + 1)
            mlflow.log_metric('val_cosine_baseline_cosmean', val_metrics["cosine_baseline_cosmean"], step=epoch + 1)
            mlflow.log_metric('val_cosine_gain_cosmean', val_metrics["cosine_gain_cosmean"], step=epoch + 1)
            mlflow.log_metric('val_cosine_baseline_cosmax', val_metrics["cosine_baseline_cosmax"], step=epoch + 1)
            mlflow.log_metric('val_cosine_gain_cosmax', val_metrics["cosine_gain_cosmax"], step=epoch + 1)
