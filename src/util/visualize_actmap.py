...
def visactmap(
    ...
    actmap_dir = osp.join(save_dir, 'actmap_' + dataset_name)
    if not os.path.exists(actmap_dir):
        os.makedirs(actmap_dir)
    print('Visualizing activation maps for {} ...'.format(dataset_name))

    for batch_idx, (imgs, labels, scan_id, perspective) in enumerate(data_loader, desc="Generate actmap"):
