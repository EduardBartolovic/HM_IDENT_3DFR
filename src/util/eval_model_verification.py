...
def evaluate_verification_lfw(device, backbone, data_root, dataset_path, writer, epoch, num_epoch, distance_metric, test_transform, batch_size):
...
def evaluate_verification_colorferet(device, backbone, data_root, dataset_path, writer, epoch, num_epoch,
                                     distance_metric, test_transform, batch_size):
    print(colorstr('bright_green', f"Perform 1:1 Evaluation on {dataset_path}"))
    dataset = ImageFolderWithVoting(os.path.join(data_root, dataset_path), transform=test_transform)
    data_loader = DataLoader(dataset, batch_size=batch_size*2, shuffle=False)
    embeddings_storage = {}
    with torch.no_grad():
