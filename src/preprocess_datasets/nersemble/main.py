...
if __name__ == '__main__':
    ...
    print("##### Analyse Video ##############")
    print("##################################")
    analyse_video_nersemble(folder_root, "analysis", model_path_hpe, face_detect_model_root, device, batch_size=batch_size, keep=True, frame_skip=frame_skip, face_confidence=0.6)
    ...
    print("##### Crop Frames ################")
    print("##################################")
    face_crop_and_alignment(dataset_output_folder, dataset_output_folder_crop, face_factor=0.8, device='cuda' if torch.cuda.is_available() else 'cpu', resize_size=(2000, 3208), det_threshold=0.05)
    ...
    print("##################################")
    create_train_test_split(dataset_output_folder_filtered, output_test_dataset, ignore_face_corr=True)
