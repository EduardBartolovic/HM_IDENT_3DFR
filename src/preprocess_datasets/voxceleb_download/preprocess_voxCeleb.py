...
REF_FPS = 1  # fps to extract frames
REF_SIZE = 360  # Height
LOW_RES_SIZE = 400
...
parser.add_argument("--dataset", required=True, type=str, choices=('vox1', 'vox2'),
                    help="Download vox1 or vox2 dataset")
...
def preprocess_frames(dataset, image_files, save_dir, txt_metadata, landmark_est=None):
    if dataset == 'vox2':
                ...
                with torch.no_grad():
                    ...
                    if not condition:
                        try:
                            ...
                            print("crop_using_landmarks failed for ", image_file)
                            continue
                        if img is not None:
                            filename = os.path.join(save_dir, image_name)
