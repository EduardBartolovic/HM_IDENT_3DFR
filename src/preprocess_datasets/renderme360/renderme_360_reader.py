...
if __name__ == '__main__':
    actor_part = "0019"
    root_dir = f'E:\\Download\\RenderMe360\\{actor_part}\\'
    ...
    for i in files:
        ...
        if len(splitted) == 3:
            ...
            parts.append(splitted[1]+"_"+splitted[2])
        else:
            raise Exception()
    print(parts)
    for part in parts:
        st = time.time()
        rd = SMCReader(root_dir+f'{actor_part}_{part}_raw.smc')
        output_dir = f"E:\\Download\\RenderMe360\\{actor_part}\\images"
        ...
        # img
        for Camera_id in range(14, 35):
            images = rd.get_img(Camera_id, 'color', disable_tqdm=True)
            for i, img in enumerate(images):
                filename = os.path.join(output_dir, f"{part}_camera{str(Camera_id)}_frame{i:04d}.png")
