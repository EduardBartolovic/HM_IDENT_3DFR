...
def analyse_video_vox(input_folder, output_folder, model_path_hpe, model_path_blazeface, device, batch_size=64, filter=None, keep=True):
        ...
        for i in range(0, len(imgs), batch_size):
            ...
            if batch:
                ...
                for image in batch:
                    ...
                    padded_image = cv2.cvtColor(
                        cv2.copyMakeBorder(image, pad_size, pad_size, pad_size, pad_size, cv2.BORDER_CONSTANT, value=[0, 0, 0]), cv2.COLOR_BGR2RGB)
